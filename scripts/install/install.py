...
def run_tui():
	...
	if not check_terminfo(original_term):
		...
		if 'ghostty' in original_term.lower():
			...
			print("  2. Install Ghostty's terminfo for full feature support")
			print()
			print("To install Ghostty terminfo on this or remote machines:")
			print(f"  {Colors.YELLOW}./scripts/utils/install-ghostty-terminfo.sh{Colors.NC}")
			print()
			print("Or manually:")
			print("  tic -x <(curl -s https://ghostty.org/terminfo)")
			print()
